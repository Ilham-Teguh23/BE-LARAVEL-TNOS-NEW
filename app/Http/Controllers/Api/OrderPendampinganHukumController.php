...
use App\Models\OrderPendampinganHukum;
...
class OrderPendampinganHukumController extends Controller
{
    protected $serverKey;
        ...
        try {
            ...
            if(in_array($date_now->format('Y-m-d'),$tanggal_libur)){
                ...
                if(Carbon::parse($format_time_now)->between($normal_shift_start,$normal_shift_end)){
                    ...
                    $waktu_kerja = '1';
                    $harga_dasar = 4000000 * 1.30;
                    $harga_total = $harga_dasar * ($request->duration / 8);
                }
                ...
                if($date_now->isWeekend()){
                    ...
                    if(Carbon::parse($format_time_now)->between($normal_shift_start,$normal_shift_end)){
                        var_dump('normal shift');
                        $waktu_kerja = '1';
                        $harga_dasar = 4000000 * 1.15;
                        $harga_total = $harga_dasar * ($request->duration / 8);
                    }
                    elseif(Carbon::parse($format_time_now)->between($night_shift_start_s,$night_shift_end_s) || Carbon::parse($format_time_now)->between($night_shift_start_p,$night_shift_end_p)){
                        var_dump('night shift');
                        $waktu_kerja = '2';
                        $harga_dasar = 4000000 * 1.2;
                        $harga_total = $harga_dasar * ($request->duration / 8);
                    }else{
                        var_dump('ada yang error');
                        // return response()->json([
                    ...
                    if(Carbon::parse($format_time_now)->between($normal_shift_start,$normal_shift_end)){
                        var_dump('normal shift');
                        $waktu_kerja = '1';
                        $harga_dasar = 4000000 * 1;
                        $harga_total = $harga_dasar * ($request->duration / 8);
                    }
                    elseif(Carbon::parse($format_time_now)->between($night_shift_start_s,$night_shift_end_s) || Carbon::parse($format_time_now)->between($night_shift_start_p,$night_shift_end_p)){
                        var_dump('night shift');
                        $waktu_kerja = '2';
                        $harga_dasar = 4000000 * 1.1;
                        $harga_total = $harga_dasar * ($request->duration / 8);
                    }else{
                        var_dump('ada yang error');
                        // return response()->json([
            ...
            //     $harga_total = 12500 *  $request->duration;
            // } else {
            //     $harga_total = 15000 *  $request->duration;
            // }

            // $dataOrderPendampinganHukum = [
            ...
            // $order = Order::create($dataOrderPendampinganHukum);
